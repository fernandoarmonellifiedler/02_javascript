...
function maximumToys(prices, k) {
  let newPrices = [...prices].filter((item) => item < k);

  if (newPrices.reduce((acc, item) => acc + item) < k) {
    return newPrices.length;
  } else {
    let sorted = newPrices.sort((a, b) => a - b);
    while (sorted.reduce((acc, item) => acc + item) > k) {
      sorted.pop();
    }
...
console.log(maximumToys([1, 12, 5, 30, 111, 200, 1000, 10], 50));
